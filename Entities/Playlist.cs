...
namespace ExtremeInsiders.Entities
{
    public class Playlist : EntitySaleable, ITranslatableEntity<Playlist, PlaylistTranslation>
    {
        ...
        [NotMapped]
        [JsonIgnore]
        public Culture OnlyLocalization => Videos != null && Videos.Count != 0
            ? Videos.All(x => x.Translations.Count == 1 && x.Translations.First().Culture.Key == Videos.First().Translations.First().Culture.Key)
                ? Videos?.First().Translations.First().Culture
                : null
            : null;
    }
